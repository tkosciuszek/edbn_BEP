...
def run(file, dataName, updateInt, updateWindow, trainPerc=0.5):
    ...
    for _, event in data.iterrows():
                    ...
                    if i not in drifts.keys():
                        ...
                        if _ > round(numEvents * trainPerc):
                            ...
                            result, timing, basic_model = m.test_and_update_indices(basic_model, d,
                                                                                    ...
                                                                                    reset=True)
                            if is_written:
                                store_resultsa("results/%s_%s_%s_%s.csv" % (m.name, d.name, updateInt, updateWindow),
                                               result)
                                store_timinga(
                                    "results/%s_%s_%s_%s_time.csv" % (m.name, d.name, updateInt, updateWindow), timing)
                            else:
                                store_results("results/%s_%s_%s_%s.csv" % (m.name, d.name, updateInt, updateWindow),
                                              result)
                                store_timing("results/%s_%s_%s_%s_time.csv" % (m.name, d.name, updateInt, updateWindow),
                                             timing)
                            is_written = 1
                            testInd = _
        ...
        if (_ - testInd > updateInt) and (_ > round(numEvents * trainPerc)):
            ...
            if is_written:
                store_resultsa("results/%s_%s_%s_%s.csv" % (m.name, d.name, updateInt, updateWindow), result)
                store_timinga("results/%s_%s_%s_%s_time.csv" % (m.name, d.name, updateInt, updateWindow), timing)
            else:
                store_results("results/%s_%s_%s_%s.csv" % (m.name, d.name, updateInt, updateWindow), result)
                store_timing("results/%s_%s_%s_%s_time.csv" % (m.name, d.name, updateInt, updateWindow), timing)
            is_written = 1
...
if __name__ == "__main__":
    intervals = [1000, 7150, 87400]#[1000, 7150, 87400, 2000, 1500, 2000, 1500, 2200, 92500, 10000]
    windows = [1000, 7150, 87400]#[1000, 7150, 87400, 2000, 1500, 2000, 1500, 2200, 92500, 1000]
