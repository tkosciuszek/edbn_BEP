...
#--------------END TESTING CODE -------------------------------------------------------------
    ...
    for ev in static_event_stream:
        ...
        # eventHist = np.array(eventHist, [ev['concept:name'], ev['lifecycle:transition'], ev['org:resource'], ev['time:timestamp'], ev['task'], ev['user'], ev['case:concept:name'], ev['case:variant'],
        #                                  ev['case:variant-index'], ev['case:creator']])
