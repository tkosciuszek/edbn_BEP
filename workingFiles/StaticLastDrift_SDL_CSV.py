...
def run(file, dataName, updateInt, updateWindow, trainPerc=0.5):
    ...
    lastUpdate = round(numEvents * trainPerc)
    drifts = {}
    start_time = time.time()
    for _, event in data.iterrows():
                        ...
                        if _ > round(numEvents * trainPerc):
                            ...
                            result, timing, basic_model = m.test_and_update_indices(basic_model, d,
                                                                                    (_ - round(numEvents * trainPerc),
                                                                                     max(testInd - round(numEvents * trainPerc),
                                                                                         0)),
                                                                                    lastUpdate - round(
                                                                                        numEvents * trainPerc),
                                                                                    _ - round(numEvents * trainPerc),
                            ...
                            if is_written:
                                store_resultsa(
                                    "results/%s_%s_StaticLast_%s_%s.csv" % (m.name, d.name, updateInt, updateWindow),
                                    result)
                                store_timinga(
                                    "results/%s_%s_StaticLast_%s_%s_time.csv" % (
        ...
        if (_ - lastUpdate > updateInt) and (_ > round(numEvents * trainPerc)):
            ...
            if is_written:
                store_resultsa("results/%s_%s_StaticLast_%s_%s.csv" % (m.name, d.name, updateInt, updateWindow), result)
                store_timinga("results/%s_%s_StaticLast_%s_%s_time.csv" % (m.name, d.name, updateInt, updateWindow),
                              timing)
            else:
                store_results("results/%s_%s_StaticLast_%s_%s.csv" % (m.name, d.name, updateInt, updateWindow), result)
                store_timing("results/%s_%s_StaticLast_%s_%s_time.csv" % (m.name, d.name, updateInt, updateWindow),
                             timing)
            is_written = 1
            lastUpdate = _
    ...
    print("Test ind is {}".format(testInd))
    print("Highest index calculation on test is {}".format(_ - round(numEvents * trainPerc)))
    result, timing, basic_model = m.test_and_update_indices(basic_model, d, (_ - round(numEvents * trainPerc),
                                                                                     max(testInd - round(numEvents * trainPerc),
                                                                                         0)),
    ...
    if is_written:
        store_resultsa("results/%s_%s_StaticLast_%s_%s.csv" % (m.name, d.name, updateInt, updateWindow), result)
        store_timinga("results/%s_%s_StaticLast_%s_%s_time.csv" % (m.name, d.name, updateInt, updateWindow), timing)
    else:
        store_results("results/%s_%s_StaticLast_%s_%s.csv" % (m.name, d.name, updateInt, updateWindow), result)
        store_timing("results/%s_%s_StaticLast_%s_%s_time.csv" % (m.name, d.name, updateInt, updateWindow), timing)

    end_time = time.time()


if __name__ == "__main__":
    intervals = [1000, 7150, 87400] #[1000, 7150, 87400, 2000, 1500, 2000, 1500, 2200, 92500]
    windows = [1000, 7150, 87400] #[1000, 7150, 87400, 2000, 1500, 2000, 1500, 2200, 92500]
    files = ['Data/Helpdesk.csv', 'Data/BPIC11.csv', 'Data/BPIC12.csv']#['Data/Helpdesk.csv', 'Data/BPIC11.csv', 'Data/BPIC12.csv', 'Data/BPIC15_1_sorted_new.csv',
             #'Data/BPIC15_2_sorted_new.csv', 'Data/BPIC15_3_sorted_new.csv', 'Data/BPIC15_4_sorted_new.csv',
             #'Data/BPIC15_5_sorted_new.csv', 'Data/BPIC17_FULL.csv']
    # intervals = [92500]
    # windows = [92500]
    # files = ['Data/BPIC17_FULL.csv']
    names = ['Helpdesk', 'BPIC11', 'BPIC12'] #['Helpdesk', 'BPIC11', 'BPIC12', 'BPIC15_1', 'BPIC15_2', 'BPIC15_3', 'BPIC15_4',
             #'BPIC15_5','BPIC17FULL']
    # for i in intervals:
    #     for w in windows:
    ...
    for i in range(len(intervals)):
        run(files[i], names[i], intervals[i], windows[i])
